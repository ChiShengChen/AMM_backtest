...
AMM Rebalance Backtester 現在使用新的組織化報告結構，每次實驗的結果都會整齊地存放在各自的資料夾中，不再散落在各處。
...
實驗資料夾名稱格式：`{POOL}_{FREQUENCY}_{DATE_RANGE}_{RUN_ID}`

例如：`ETHUSDC_1d_20240101_20240131_a1b2c3d4`
...
- **apr_mdd_scatter_{run_id}.png**: APR vs MDD 散點圖
- **fee_vs_price_pnl_{run_id}.png**: 費用 vs 價格 PnL 分析
- **sensitivity_heatmap_{run_id}.png**: 參數敏感性熱圖
- **gas_frequency_contour_{run_id}.png**: Gas 費用與頻率分析
- **il_curve_{run_id}.png**: 無常損失曲線
- **lvr_estimates_{run_id}.png**: LVR 估算分析

### data/ 資料夾
- **strategy_results_{run_id}.csv**: 策略結果數據（CSV格式）
...
1. 首先掃描現有文件：`python cleanup_amm_reports.py scan`
2. 預覽組織化操作：`python cleanup_amm_reports.py organize --dry-run`
3. 執行組織化：`python cleanup_amm_reports.py organize`
4. 或者使用完整清理：`python cleanup_amm_reports.py cleanup`
