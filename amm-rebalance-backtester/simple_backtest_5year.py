"""
簡化版5年日線數據回測腳本 - AMM Rebalance Backtester
...
def simulate_strategy_performance(df: pd.DataFrame, symbol: str, strategy_name: str, strategy_type: str) -> dict:
    ...
    if "Quantum" in strategy_name:
        ...
        rebalance_prob = 0.15  # 15%概率再平衡
        base_width = 0.08  # 8%基礎寬度
        volatility_factor = 1.5
    else:
        # 經典ML策略：較少再平衡，較寬的位置寬度
        rebalance_prob = 0.08  # 8%概率再平衡
        base_width = 0.12  # 12%基礎寬度
    ...
    for i in range(50, len(df)):
        ...
        width = max(0.05, min(0.3, width))  # 限制在5%-30%之間
        ...
        if i > 0:
            ...
            # 假設策略能捕捉部分價格變動
            portfolio_change = price_change * 0.3  # 30%的價格變動轉化為投資組合變動
...
def run_all_strategies(data: dict):
    ...
    # 定義策略
    quantum_strategies = [
        'QuantumBased', 'QuantumVolatility', 'QuantumHybrid'
    ]
    
    classic_ml_strategies = [
        'MLBased', 'MLVolatility', 'MLHybrid'
    ]
