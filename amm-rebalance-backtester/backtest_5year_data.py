"""
5年日線數據回測腳本 - AMM Rebalance Backtester
...
def load_data(data_dir: str = "data/5year_daily") -> dict:
    """加載5年日線數據"""
    data_files = {
...
def run_quantum_strategies(data: dict):
    ...
    try:
        ...
        for symbol, df in data.items():
            ...
            # 測試不同的量子策略
            strategies = {
                'QuantumBased': QuantumBasedStrategy(
                ...
                'QuantumVolatility': QuantumVolatilityStrategy(
...
def run_classic_ml_strategies(data: dict):
    ...
    try:
        ...
        from src.ml import MLStrategy, FeatureEngineer, RebalancePredictor, VolatilityPredictor
        ...
        for symbol, df in data.items():
            ...
            rebalance_predictor = RebalancePredictor()
            volatility_predictor = VolatilityPredictor()
            ml_strategy = MLStrategy(feature_engineer, rebalance_predictor)
            
            # 測試不同的經典ML策略
            strategies = {
                'MLBased': MLBasedStrategy(
                    ml_strategy=ml_strategy,
                    initial_width=0.1,
                    rebalance_cooldown_hours=24
                ),
                'MLVolatility': MLVolatilityStrategy(
                    volatility_model=volatility_predictor,
                ...
                'MLHybrid': MLHybridStrategy(
                    rebalance_model=rebalance_predictor,
                    volatility_model=volatility_predictor,
                    feature_engineer=feature_engineer,
                    rebalance_weight=0.6,
                    volatility_weight=0.4,
