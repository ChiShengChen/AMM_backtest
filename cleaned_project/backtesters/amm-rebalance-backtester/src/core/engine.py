...
class BacktestEngine:
    ...
    def generate_organized_reports(self, output_dir: str = "reports") -> Dict[str, str]:
        """
        Generate organized reports and charts for this backtest.
        ...
        """
        from ..reporting.organized_reports import OrganizedReportGenerator
        
        # Add pool and frequency info to results
        self.results['pool'] = self.config.get('pool', 'Unknown')
        self.results['frequency'] = self.config.get('frequency', 'Unknown')
        
        generator = OrganizedReportGenerator(self.results, output_dir)
        report_files = generator.generate_all_reports()
        
        logger.info(f"Generated organized reports for pool {self.results['pool']}")
        logger.info(f"Experiment directory: {generator.experiment_dir}")
