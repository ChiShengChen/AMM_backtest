"""
Demo script for quantum machine learning strategies in Steer Intent backtester.
"""
...
def test_quantum_models():
    ...
    try:
        ...
        # Create features
        from steerbt.ml import SteerFeatureEngineer
        ...
        y_intent = (features_df['close'].pct_change().abs() > 0.01).astype(int).values[1:]
        y_price = (features_df['close'].rolling(20).std() > features_df['close'].rolling(20).std().mean()).astype(int).values[20:]
        
        # Adjust X to match y lengths
        X_intent = X[1:len(y_intent)+1]
        X_price = X[20:len(y_price)+20]
        
        logger.info(f"Training data shapes: X_intent={X_intent.shape}, y_intent={y_intent.shape}")
        logger.info(f"Training data shapes: X_price={X_price.shape}, y_price={y_price.shape}")
        
        # Test Steer QNN Intent Predictor
        try:
            logger.info("Testing Steer QNN Intent Predictor...")
            qnn_model = SteerQNNIntentPredictor(n_qubits=2, n_layers=1, feature_dim=4)
            ...
            logger.info(f"Steer QNN Predictions: {predictions}")
            logger.info(f"Steer QNN Probabilities shape: {probabilities.shape}")
            
        except Exception as e:
            logger.error(f"Steer QNN test failed: {e}")
        
        # Test Steer QSVM Price Predictor
        try:
            logger.info("Testing Steer QSVM Price Predictor...")
            qsvm_model = SteerQSVMPricePredictor(n_qubits=2, feature_dim=4)
            ...
            logger.info(f"Steer QSVM Predictions: {predictions}")
            logger.info(f"Steer QSVM Probabilities shape: {probabilities.shape}")
            
        except Exception as e:
            logger.error(f"Steer QSVM test failed: {e}")
...
def test_quantum_strategies():
    ...
    try:
        ...
        # Test QuantumBollingerStrategy
        try:
            logger.info("Testing QuantumBollingerStrategy...")
            strategy = QuantumBollingerStrategy(
            ...
            logger.info(f"QuantumBollingerStrategy - Rebalance decisions: {sum(rebalance_decisions)}/{len(rebalance_decisions)}")
            logger.info(f"QuantumBollingerStrategy - Avg position width: {np.mean(position_widths):.3f}")
            
            # Get strategy info
            strategy_info = strategy.get_strategy_info()
            logger.info(f"QuantumBollingerStrategy info: {strategy_info}")
            
        except Exception as e:
            logger.error(f"QuantumBollingerStrategy test failed: {e}")
        
        # Test QuantumKeltnerStrategy
        try:
            logger.info("Testing QuantumKeltnerStrategy...")
            strategy = QuantumKeltnerStrategy(
            ...
            logger.info(f"QuantumKeltnerStrategy - Rebalance decisions: {sum(rebalance_decisions)}/{len(rebalance_decisions)}")
            logger.info(f"QuantumKeltnerStrategy - Avg position width: {np.mean(position_widths):.3f}")
            
            # Get strategy info
            strategy_info = strategy.get_strategy_info()
            logger.info(f"QuantumKeltnerStrategy info: {strategy_info}")
            
        except Exception as e:
            logger.error(f"QuantumKeltnerStrategy test failed: {e}")
